...
public class AgentManager {
    ...
    public synchronized void addAgent(Bundle bundle, Agent agent, boolean created) {
        String bundleIdentifier = bundle.getSymbolicName() + BUNDLE_NAME_VERSION_SEPARATOR
    ...
    public synchronized void removeAgent(AID aid) {
        found: for (Entry<String, List<AgentWrapper>> entry : agents.entrySet()) {
            ...
            for (AgentWrapper aw : agentWrappers) {
                ...
                if (aw.created) {
                    ...
                    AgentInfo ai = new AgentInfo(a.getLocalName(), a.getClass().getName(), a.getArguments(),
                            b.getSymbolicName(), b.getHeaders().get(Constants.BUNDLE_VERSION));
        ...
        if (agentsToRestart.containsKey(bundleIdentifier)) {
            ...
            if (jrs != null) {
                for (AgentInfo ai : agentsToRestart.get(bundleIdentifier)) {
                    try {
                        if (ai.updated) {
                            ...
                            AgentController ac = jrs.createNewAgent(ai.name, ai.className, ai.args, ai.symbolicName,
                                    ai.version);
                            ac.start();
                        }
                    } catch (Exception e) {
                ...
                logger.log(Logger.WARNING,
                        "JadeRuntimeService for " + bundleIdentifier + " no more active! Cannot restart agents!");
            }
